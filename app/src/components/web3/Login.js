...
const init = async (dispatch) => {
  ...
  await dispatch({ type: 'SET_ERC20', payload: await new web3.eth.Contract(CD.ERC20_ABI, CD.LINK) });
  await dispatch({ type: 'SET_ICFACTORY', payload: await new web3.eth.Contract(CD.ICFACTORY_ABI, CD.ICFACTORY) });
  await dispatch({ type: 'SET_NETID', payload: await web3.eth.net.getId() })
}
...
const ConnectButton = () => {
  ...
  const [style, setStyle] = useState('connect-button primary')
  ...
  useEffect(async () => {
    if (state.account) {
      setMessage(<>{'Logged in as: ' + state.account.substring(0, 6) + '...' + state.account.substring(38,)}</>)
      setButtonVariant("secondary")
      setStyle("connect-button small-text secondary")
    }
  }, [state.account]);

  return (
    <>
      <Button className={style} variant={buttonVariant} onClick={onClickHandler}>{message}</Button>
    </>
  );
}
